...
1. On the tabs click **Infra**. There are three vCPU tier graphs:
    ...
    * The second is the graph which shows vCPU tier view over timeline (need to select a timeline greater than 24 hours but not greater than two weeks). This frame shows the vCPU view across the selected timeframe for more than 24 hours, up to two weeks. However, this graph will only show one sample per day and will not be accurate if cluster size changes multiple times in a day.

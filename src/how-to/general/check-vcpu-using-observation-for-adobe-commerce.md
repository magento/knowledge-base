...
1. On the tabs click **Infra**. There are three vCPU tier graphs:
    * The first is the graph which shows vCPU tier view over timeline greater than two weeks (You will need to select a timeline greater than two weeks). NOTE: This will only show one sample per day and will not be accurate if cluster size changes multiple times in a day.

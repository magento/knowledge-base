---
title: "MDVA-39713: User is able to edit start time for active scheduled update"
labels: QPT patches,Quality Patches Tool,Support Tools,QPT 1.1.12,scheduled update,error,Magento,Adobe Commerce,cloud infrastructure,on-premises,2.3.0,2.3.1,2.3.2,2.3.2-p2,2.3.3,2.3.3-p1,2.3.4,2.3.4-p2,2.3.5,2.3.5-p1,2.3.5-p2,2.3.6
---
...
1. Activate by using `bin/magento cron:run --group=staging` in local environment. Wait for some time and check if the schedule is active.
1. After the schedule is active, refresh the page.
1. Click on **View/Edit** in **Scheduled Changes**.
1. Edit the time, add +2 minutes and save.
1. Save the CMS page.
1. Again run `bin/magento cron:run --group=staging`.
1. Click on **View/Edit** for a schedule update.

...
<h5>Step 3</h5>
<div class="zd-accordion-section">On Redis versions 3.2 or 5.0?Check by running the following command in the CLI: <code>$ redis-cli -p %port-number% info | grep redis_version</code>// Pro, where %port-number% is the number of the port, which can be found in the `app/etc/env.php` file or by running one of these commands: <code>vendor/bin/ece-tools env:config:show |grep -i redis -A 3</code> or <code>cat app/etc/env.php |grep redis -A 3</code> . For Staging run <code>$ redis-cli -h 'redis.internal' info | grep redis_version</code>// Starter or Integration</div>

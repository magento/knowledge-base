...
<p class="zd-accordion-text">a. YES – You cannot use Advanced Reporting, as we only support one currency. <br>b. NO – Output shows only one currency. Example: <code>USD</code>. Then check have multiple base currencies ever been used (in orders)? Run this SQL command:<br>
